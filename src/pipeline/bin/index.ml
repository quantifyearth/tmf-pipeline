...
let or_fail label x =
  ...
  | Sqlite3.Rc.OK -> ()
  | err ->
      Fmt.failwith "Sqlite344 %s error: %s" label (Sqlite3.Rc.to_string err)
...
CREATE TABLE IF NOT EXISTS deployer_index (
  ...
  PRIMARY KEY (owner, name, hash, variant)
     ...
     let record_job =
       Sqlite3.prepare db
         "INSERT OR REPLACE INTO deployer_index (owner, name, hash, variant, \
          job_id) VALUES (?, ?, ?, ?, ?)"
     in
     let remove =
       Sqlite3.prepare db
         "DELETE FROM deployer_index WHERE owner = ? AND name = ? AND hash = ? \
          AND variant = ?"
     in
     let get_job_ids =
       Sqlite3.prepare db
         "SELECT variant, job_id FROM deployer_index WHERE owner = ? AND name \
          = ? AND hash = ?"
     in
     { record_job; remove; get_job_ids })
    ...
    let set job_id =
      ...
      | None ->
          Db.exec t.record_job
            Sqlite3.Data.
              [ TEXT owner; TEXT name; TEXT hash; TEXT variant; NULL ]
      | Some id ->
          Db.exec t.record_job
            Sqlite3.Data.
              [ TEXT owner; TEXT name; TEXT hash; TEXT variant; TEXT id ]
    in
    let update j1 j2 =
      match (j1, j2) with
